...
instance Print AbsCPP.Def where
  prt i e = case e of
    ...
    AbsCPP.DFunI type_ id args -> prPrec i 0 (concatD [prt 0 type_, prt 0 id, doc (showString "("), prt 0 args, doc (showString ")")])
  prtList _ [] = concatD []
...
instance Print AbsCPP.Stm where
  prt i e = case e of
    AbsCPP.SExp exp -> prPrec i 0 (concatD [prt 0 exp])
    AbsCPP.SDecl type_ id -> prPrec i 0 (concatD [prt 0 type_, prt 0 id])
    AbsCPP.Secls type_ id ids -> prPrec i 0 (concatD [prt 0 type_, prt 0 id, doc (showString ","), prt 0 ids])
    AbsCPP.SInit type_ id exp -> prPrec i 0 (concatD [prt 0 type_, prt 0 id, doc (showString "="), prt 0 exp])
    AbsCPP.SReturn exp -> prPrec i 0 (concatD [doc (showString "return"), prt 0 exp])
...
instance Print AbsCPP.Exp where
  prt i e = case e of
    AbsCPP.EIndex exp n -> prPrec i 16 (concatD [prt 15 exp, doc (showString "["), prt 0 n, doc (showString "]")])
    AbsCPP.EDeref exp -> prPrec i 16 (concatD [doc (showString "*"), prt 15 exp])
    AbsCPP.EAddr exp -> prPrec i 16 (concatD [doc (showString "&"), prt 15 exp])
    ...
    AbsCPP.ECall id exps -> prPrec i 15 (concatD [prt 0 id, doc (showString "("), prt 0 exps, doc (showString ")")])
    ...
    AbsCPP.ENeg exp -> prPrec i 13 (concatD [doc (showString "-"), prt 14 exp])
    AbsCPP.EMul exp1 exp2 -> prPrec i 12 (concatD [prt 12 exp1, doc (showString "*"), prt 13 exp2])
    AbsCPP.EDiv exp1 exp2 -> prPrec i 12 (concatD [prt 12 exp1, doc (showString "/"), prt 13 exp2])
    AbsCPP.EAdd exp1 exp2 -> prPrec i 11 (concatD [prt 11 exp1, doc (showString "+"), prt 12 exp2])
    AbsCPP.ESub exp1 exp2 -> prPrec i 11 (concatD [prt 11 exp1, doc (showString "-"), prt 12 exp2])
...
instance Print AbsCPP.Type where
  prt i e = case e of
    AbsCPP.Tarr type_ -> prPrec i 0 (concatD [prt 0 type_, doc (showString "[]")])
    AbsCPP.Tarrs type_ n -> prPrec i 0 (concatD [prt 0 type_, doc (showString "["), prt 0 n, doc (showString "]")])
    AbsCPP.Taddress type_ -> prPrec i 0 (concatD [doc (showString "&"), prt 0 type_])
    AbsCPP.Tpointer type_ -> prPrec i 0 (concatD [doc (showString "*"), prt 0 type_])
    ...
    AbsCPP.Tdef id -> prPrec i 0 (concatD [prt 0 id])
